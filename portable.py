...
working_directory = input("Enter the working directory: ")
print("")

hl_choice = 0
hl_correct_answers = []

while hl_choice not in [1, 2]:
    hl_choice = int(input("Choose: 1 - enter HL correct answers as a text, 2 - enter HL correct answers as a file: "))
    if hl_choice == 1:
        hl_correct_answers = list(re.sub(r'[^A-D]', '', input("Enter HL correct answers as a text: ").upper()))
    elif hl_choice == 2:
        with open(os.path.join(working_directory, input("Enter HL correct answers file name: "))) as file:
            hl_correct_answers = list(re.sub(r'[^A-D]', '', file.read().upper()))
    else:
        print("Wrong input")
print("")
...
while sl_choice not in [1, 2]:
    sl_choice = int(input("Choose: 1 - enter SL correct answers as a text, 2 - enter SL correct answers as a file: "))
    if sl_choice == 1:
        sl_correct_answers = list(re.sub(r'[^A-D]', '', input("Enter SL correct answers as a text: ").upper()))
    elif sl_choice == 2:
        with open(os.path.join(working_directory, input("Enter SL correct answers file name: ")), 'r') as file:
            sl_correct_answers = list(re.sub(r'[^A-D]', '', file.read().upper()))
    else:
        print("Wrong input")
print("")

with open(os.path.join(working_directory, input("Enter input file name: ")), 'rb') as file:
    with open(os.path.join(working_directory, input("Enter output file name: ")), 'wb') as output:
        output.write(grade(file.read(), [hl_correct_answers, sl_correct_answers], debug=True))